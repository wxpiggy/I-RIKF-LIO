...
void IESKF::predict(IMU &imu, double dt) {
    ...
    X.rotation = Eigen::Quaterniond(X.rotation.toRotationMatrix() * math::so3Exp((imu.gyroscope) * dt));
    ...
    Fw.setZero();
    Fx.setIdentity();
    Fx.block<3, 3>(0, 0) = math::so3Exp(-1 * imu.gyroscope * dt);

    Fx.block<3, 3>(0, 9) = -1 * math::A_T(-imu.gyroscope * dt) * dt;

    Fx.block<3, 3>(3, 6) = Eigen::Matrix3d::Identity() * dt;
    Fx.block<3, 3>(6, 0) = rotation * math::skewSymmetric(imu.acceleration) * dt * (-1);
    Fx.block<3, 3>(6, 12) = rotation * dt * (-1);
    Fx.block<3, 3>(6, 15) = Eigen::Matrix3d::Identity() * dt;
    Fw.block<3, 3>(0, 0) = -1 * math::A_T(-imu.gyroscope * dt) * dt;
...
bool IESKF::update() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        J_inv.setIdentity();
        J_inv.block<3, 3>(0, 0) = math::A_T(error_state.block<3, 1>(0, 0));
        // 更新 P
        ...
        // 更新X
        x_k_k.rotation = x_k_k.rotation.toRotationMatrix() * math::so3Exp(update_x.block<3, 1>(0, 0));
...
Eigen::Matrix<double, 18, 1> IESKF::getErrorState18(const State18 &s1, const State18 &s2) {
    ...
    es.setZero();
    es.block<3, 1>(0, 0) = math::SO3Log((s2.rotation.conjugate() * s1.rotation).toRotationMatrix());
